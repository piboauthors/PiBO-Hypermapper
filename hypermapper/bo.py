...
from hypermapper.priors import Prior
# ensure backward compatibility
try:
    from hypermapper import models
    from hypermapper import space
    from hypermapper.priors import Prior, process_prior
...
def main(config, black_box_function=None, profiling=None):
    ...
    """
    if 'hpo' not in config["application_name"]:
        application_name = config["application_name"].split('_')[0]
    else:
        application_name = config["application_name"]
    beta = 0
    use_prior = True
    print('Prior is used with a beta of ', beta)
    try:
        prior_strength = sys.argv[3]
    except:
        prior_strength = 'strong'
    prior_index = sys.argv[2]
    ...
    # TODO here, we add the prior object and the associated beta
    prior_path = join(dirname(dirname(__file__)), f'prior_jsons/{application_name}/{prior_strength}/prior{prior_index}.json')
    application_name = f'{application_name}_{prior_strength}_run_{sys.argv[2]}'

    if use_prior:
        with open(prior_path, 'r') as f:
            prior_dict =json.load(f)

        prior_args = process_prior(prior_dict)
        prior = Prior(param_space, *prior_args)
    ...
    if absolute_configuration_index < number_of_doe_samples:
        ...
        if prior is None:
            ...
            mode = prior.get_max_location(as_config=True)
            configurations += mode
            if beta == 0:
                doe_configurations = param_space.get_doe_sample_configurations(fast_addressing_of_data_array, number_of_doe_samples -1, doe_type)
                prior = None
                
            else:
                doe_configurations = prior.sample(number_of_doe_samples-1, normalize=False, as_config=True)
            configurations += doe_configurations
            print(configurations)
        print(
            "Design of experiments phase, number of new doe samples = %d ......."
            % len(configurations)
        )
    ...
    if enable_feasible_predictor:
        while (
            ...
            if prior is None:
                ...
                configurations = prior.sample(1, normalize=False, as_config=True)
    ...
    while evaluation_count < evaluation_budget and run_time / time_budget < 1:
        ...
        if epsilon > epsilon_greedy_threshold:
            ...
            if prior is not None:
                print('Sampled the configuration from the prior.')
                best_configuration = prior.sample(1, normalize=False, as_config=True)[0]
