...
def run_acquisition_function(
    ...
    if acquisition_function == "TS":
    ...
    elif acquisition_function == "PriorEI":
        scalarized_values, tmp_objective_limits = EI(
            configurations,
            data_array,
            objective_weights,
        ...
        prior_values = prior(np.array(configurations), normalized_input=False)
        weighted_prior_values = np.power(prior_values+1e-6, beta/iteration_number)
        scalarized_values  = scalarized_values * weighted_prior_values.reshape(-1)
...
def random_scalarizations(
    ...
    optimization_function_parameters["prior"] = prior
    optimization_function_parameters["beta"] = beta
    if prior is not None:
        optimization_function_parameters["acquisition_function"] = 'PriorEI'
